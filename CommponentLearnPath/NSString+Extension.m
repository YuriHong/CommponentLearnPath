...
//        NSString *format = isYearSame ? @"MM-dd" : @"YYYY-MM-dd";
...
+ (NSString *)filterEmoji:(NSString *)string {
    ...
    NSString * result = [regex stringByReplacingMatchesInString:string options:NSMatchingReportProgress range:NSMakeRange(0, string.length) withTemplate:@""];
    
    return result;
}

//字符串中是否包含表情字符
+ (BOOL)hasEmoji:(NSString* _Nonnull)string{
...
+ (NSMutableAttributedString *_Nullable)setFirstLineHeadIndent:(CGFloat)width withTitle:(NSString *_Nonnull)title font:(UIFont* _Nonnull)font
{
    NSMutableParagraphStyle *muStyle = [[NSMutableParagraphStyle alloc]init];
    muStyle.lineSpacing = 3;//设置行间距离
    muStyle.firstLineHeadIndent = width; //段落首行缩进
...
+ (NSString * _Nullable)des:(NSString * _Nonnull)str key:(NSString * _Nonnull)key isEncrypt:(BOOL)isEncrypt {
    ...
    if (isEncrypt) {
        ...
        data = [NSString convertHexStrToData:str];
    }
...
+ (NSString *)getRealName:(NSString *)userName{
    if([userName containsString:@"sinaWeibo-"]){
        return [userName substringFromIndex:10];
    }else if([userName containsString:@"weixin-"]){
        return [userName substringFromIndex:7];
    }else if ([userName containsString:@"QQ-"]){
        return [userName substringFromIndex:3];
    }else{
        return userName;
    }
}
